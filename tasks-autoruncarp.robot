...
${WATCH_FOLDER}         R:/COMPARTIDA/Robot Quadminds
${PROCESADOS_FOLDER}    R:/COMPARTIDA/Robot Quadminds/Procesados
${loop}                 True
...
Automatizacion Carga de Datos a Quadminds
    WHILE    ${loop} == True
        ${rutaArchivo}=    Escuchar carpeta
        ${disponibilidad}    ${Fecha}=    Abrir excel y guardar datos con JSON    ${rutaArchivo}
        Abrir Quadminds
        Log in
...
*** Keywords ***
Escuchar carpeta
    ${Archivo}=    Esperar Excel    ${WATCH_FOLDER}
    RETURN    ${Archivo}
...
Abrir excel y guardar datos con JSON
    ...
    RPA.Excel.Files.Open workbook    ${rutaArchivo}
    ${worksheet}=    Read worksheet    header=${TRUE}    start=2
    ${disponibilidad}=    Create table    ${worksheet}
    ${Fecha}=    Get Cell Value    1    B
    Log    ${Fecha}
    Close Workbook
    ${archivo}=    Obtener Nombre Archivo    ${rutaArchivo}
    Move File    ${rutaArchivo}    ${PROCESADOS_FOLDER}/${archivo}
